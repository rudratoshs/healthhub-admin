...
const AIClientSettings: React.FC<AIClientSettingsProps> = ({ gymId }) => {
  ...
  const defaultConfig = data?.data.find(
    (config) => config.is_default && config.status === "active"
  );

  if (isLoading) {
    return (
      <div className="flex justify-center p-4">Loading AI settings...</div>
    );
  }
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>No active AI configuration found</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="bg-muted p-4 rounded-lg text-sm">
            <p>Your gym administrator has not configured AI settings yet.</p>
            <p className="mt-2">
              This may limit some features that require AI processing.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="bg-muted p-3 rounded-lg text-sm">
            <p>
              This AI configuration is used for generating diet suggestions,
              workout plans, and analyzing assessment results.
            </p>
          </div>
        </div>
      </CardContent>
