...
import { Utensils, Calendar, Copy, Edit, Eye, Trash2, ArrowRight } from 'lucide-react';
...
const DietPlanCard: React.FC<DietPlanCardProps> = ({ plan, onDelete, onDuplicate }) => {
  ...
  return (
    <Card className="overflow-hidden transition-all duration-300 hover:shadow-md border-primary-100 h-full flex flex-col">
      <CardHeader className="bg-gradient-to-r from-primary-50 to-secondary-50 pb-4">
        <div className="flex justify-between items-start">
          <div>
            <h3 className="text-lg font-semibold text-primary-900 font-playfair line-clamp-1">{plan.title}</h3>
            <p className="text-sm text-muted-foreground line-clamp-2">{plan.description}</p>
          </div>
          <Badge className={`${getStatusBadgeVariant(plan.status)} animate-fade-in`}>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-secondary-50 p-3 rounded-md">
              ...
              <p className="text-lg font-semibold text-primary-900">{plan.daily_calories} kcal</p>
            </div>
            <div className="bg-secondary-50 p-3 rounded-md">
              ...
              <div className="flex items-center">
                ...
                <p className="text-sm font-medium">
                  {new Date(plan.end_date).getDate() - new Date(plan.start_date).getDate()} days
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium text-primary-900">Macronutrients</p>
            ...
            <div className="h-2 bg-gray-100 rounded-full overflow-hidden flex">
              ...
              <div className="bg-green-500 h-full" style={{ width: `${carbsPercentage}%` }}></div>
              <div className="bg-yellow-500 h-full" style={{ width: `${fatsPercentage}%` }}></div>
            </div>
      ...
      <CardFooter className="flex flex-wrap gap-2 bg-gray-50 pt-4">
        ...
        <Button 
          variant="outline" 
          size="sm" 
          className="flex-1 hover-scale text-destructive hover:text-destructive"
