...
const dietPlanSchema = z.object({
  ...
  daily_calories: z.coerce.number().min(500, 'Calories must be at least 500').max(10000, 'Calories must be at most 10000'),
  protein_grams: z.coerce.number().min(10, 'Protein must be at least 10g').max(500, 'Protein must be at most 500g'),
  carbs_grams: z.coerce.number().min(10, 'Carbs must be at least 10g').max(1000, 'Carbs must be at most 1000g'),
  fats_grams: z.coerce.number().min(5, 'Fats must be at least 5g').max(500, 'Fats must be at most 500g'),
...
const DietPlanForm: React.FC<DietPlanFormProps> = ({
  ...
  const totalGrams = Number(watchProtein) + Number(watchCarbs) + Number(watchFats);
  
  const proteinPercentage = totalGrams > 0 ? Math.round((Number(watchProtein) / totalGrams) * 100) : 0;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="mb-4">
                    ...
                    <FormControl>
                      <Input {...field} placeholder="e.g., Weight Loss & Detox Plan" className="bg-white" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <div className="bg-gradient-to-r from-blue-50 to-primary-50 p-4 rounded-md">
              <h3 className="text-lg font-semibold mb-2 flex items-center">
                <CalendarIcon className="h-5 w-5 mr-2 text-primary-600" /> Duration
              </h3>
            ...
            <div className="bg-gradient-to-r from-green-50 to-yellow-50 p-4 rounded-md">
              ...
              <div className="mt-6">
                ...
                <div className="mt-6 p-3 bg-white rounded-md border">
                  <h4 className="text-sm font-medium mb-2">Calculated Nutritional Values</h4>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                    <div>
                      <span className="text-muted-foreground">Protein calories:</span>
                      <span className="font-medium ml-2 text-blue-700">{proteinCalories} kcal</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Carbs calories:</span>
                      <span className="font-medium ml-2 text-green-700">{carbsCalories} kcal</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Fats calories:</span>
                      <span className="font-medium ml-2 text-yellow-700">{fatsCalories} kcal</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Total calories:</span>
                      <span className="font-medium ml-2">{totalCalories} kcal</span>
                    </div>
                  </div>
                  
                  {Math.abs(totalCalories - form.watch('daily_calories')) > 50 && (
                    <p className="text-amber-600 text-xs mt-2">
                      Note: Your macronutrient calories ({totalCalories} kcal) don't match your daily calorie target ({form.watch('daily_calories')} kcal).
                    </p>
                  )}
                </div>
              </div>
