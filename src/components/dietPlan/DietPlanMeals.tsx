...
import { getDietPlanMealPlans, createMealPlan, deleteMealPlan } from '@/lib/dietPlans';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { Loader } from '@/components/ui/loader';
import { Button } from '@/components/ui/button';
import { MealPlan, CreateMealPlanRequest } from '@/types/dietPlan';
import { Calendar, Coffee, ChefHat, Clock, PlusCircle, Utensils, Trash2 } from 'lucide-react';
...
const DietPlanMeals: React.FC<DietPlanMealsProps> = ({ dietPlanId }) => {
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreateMealPlanRequest) => createMealPlan(dietPlanId, data),
    onSuccess: () => {
      toast({
        title: "Success",
        description: "Meal plan created successfully",
      });
      queryClient.invalidateQueries({ queryKey: ['dietPlanMeals', dietPlanId] });
        ...
        <CardContent className="pt-4">
          {mealPlans.length === 0 ? (
            <div className="text-center py-12 border rounded-lg bg-gray-50">
              ...
              <p className="mb-4 text-muted-foreground">This diet plan doesn't have any meal plans yet.</p>
              <Button 
                className="bg-primary hover:bg-primary-600"
                onClick={() => setIsAddDialogOpen(true)}
              >
              ...
              <TabsList className="bg-background border-b w-full justify-start overflow-x-auto mb-4">
                {dayTabs.map((tab) => (
                  ...
                  >
                    ...
                    {tab.day_of_week ? 
                      tab.day_of_week.charAt(0).toUpperCase() + tab.day_of_week.slice(1) : 
