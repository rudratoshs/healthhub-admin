...
export const getAssessmentResult = async (id: number): Promise<AssessmentResult> => {
  try {
    const response = await api.get<{ assessment: Assessment; diet_plan: DietPlan }>(`/assessments/${id}/result`);
    const { assessment, diet_plan } = response;
    return { assessment, diet_plan };
  } catch (error) {
