...
const EditAIConfiguration: React.FC = () => {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["aiConfigurations", numericGymId],
      });
      toast({
        title: "Success",
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center p-8">Loading AI configuration...</div>
    );
  }
  ...
  if (error || !data?.data) {
    return (
      <div className="bg-destructive/10 p-4 rounded-lg text-destructive">
        ...
        {(error as Error)?.message || "Configuration not found"}
      </div>
    );
  }
